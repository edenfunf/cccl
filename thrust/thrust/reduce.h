 ...
 *  case operator+) is commutative.  If the reduction operator is not
 * commutative then \p thrust::reduce should not be used.  Instead, one could
 * use
 ...
 * href="https://en.cppreference.com/w/cpp/iterator/input_iterator">Input
 * Iterator</a> and if \c x and \c y are objects of \p InputIterator's \c
 * value_type, then <tt>x + y</tt> is defined and is convertible to \p
 * InputIterator's \c value_type. If \c T is \c InputIterator's \c value_type,
 * then <tt>T(0)</tt> is defined.
 *
 *  The following code snippet demonstrates how to use \p reduce to compute
 *  the sum of a sequence of integers using the \p thrust::host execution policy
 * for parallelization:
 *
 *  \code
...
/*! \p reduce is a generalization of summation: it computes the sum (or some
 *  other binary operation) of all the elements in the range <tt>[first,
 *  last)</tt>. This version of \p reduce uses \p init as the initial value of
 * the reduction. \p reduce is similar to the C++ Standard Template Library's
 *  <tt>std::accumulate</tt>. The primary difference between the two functions
 *  is that <tt>std::accumulate</tt> guarantees the order of summation, while
 *  \p reduce requires associativity of the binary operation to parallelize
 *  the reduction.
 ...
 * href="https://en.cppreference.com/w/cpp/iterator/input_iterator">Input
 * Iterator</a> and if \c x and \c y are objects of \p InputIterator's \c
 * value_type, then <tt>x + y</tt> is defined and is convertible to \p T.
 * \tparam T is convertible to \p InputIterator's \c value_type.
 *
 *  The following code snippet demonstrates how to use \p reduce to compute
 *  the sum of a sequence of integers including an initialization value using
 ...
 *  other binary operation) of all the elements in the range <tt>[first,
 *  last)</tt>. This version of \p reduce uses \p init as the initial value of
 * the reduction and \p binary_op as the binary function used for summation. \p
 * reduce is similar to the C++ Standard Template Library's
 * <tt>std::accumulate</tt>. The primary difference between the two functions is
 * that <tt>std::accumulate</tt> guarantees the order of summation, while \p
 * reduce requires associativity of
 *  \p binary_op to parallelize the reduction.
 *
 *  Note that \p reduce also assumes that the binary reduction operator (in this
 *  case \p binary_op) is commutative.  If the reduction operator is not
 * commutative then \p thrust::reduce should not be used.  Instead, one could
 * use
 *  \p inclusive_scan (which does not require commutativity) and select the
 *  last element of the output array.
 ...
 * and is convertible to \p BinaryFunction's first and second argument type.
 *  \tparam BinaryFunction The function's return type must be convertible to \p
 * OutputType.
 *
 *  The following code snippet demonstrates how to use \p reduce to
 *  compute the maximum value of a sequence of integers using the \p
...
/*! \p reduce_into is a generalization of summation: it computes the sum (or
 * some other binary operation) of all the elements in the range <tt>[first,
 *  last)</tt>. This version of \p reduce_into uses \c 0 as the initial value of
 * the reduction. \p reduce_into is similar to the C++ Standard Template
 * Library's <tt>std::accumulate</tt>. The primary difference between the two
 * functions is that <tt>std::accumulate</tt> guarantees the order of summation,
 * while
 *  \p reduce_into requires associativity of the binary operation to parallelize
 *  the reduction.
 *
 *  Note that \p reduce_into also assumes that the binary reduction operator (in
 * this case operator+) is commutative. If the reduction operator is not
 * commutative then \p reduce_into should not be used. Instead, one could use \p
 * inclusive_scan (which does not require commutativity) and select the last
 ...
 * Iterator</a> and \c OutputIterator's \c value_type is assignable from \c
 * InputIterator's \c value_type.
 *
 *  The following code snippet demonstrates how to use \p reduce_into to compute
 *  the sum of a sequence of integers using the \p thrust::device execution
 ...
 *  thrust::reduce_into(thrust::device, data.begin(), data.end(),
 * output.begin());
 *  // output[0] == 9
 *  \endcode
 *
...
/*! \p reduce_into is a generalization of summation: it computes the sum (or
 * some other binary operation) of all the elements in the range <tt>[first,
 *  last)</tt>. This version of \p reduce_into uses \p init as the initial value
 * of the reduction. \p reduce_into is similar to the C++ Standard Template
 * Library's <tt>std::accumulate</tt>. The primary difference between the two
 * functions is that <tt>std::accumulate</tt> guarantees the order of summation,
 * while
 *  \p reduce_into requires associativity of the binary operation to parallelize
 ...
 * some other binary operation) of all the elements in the range <tt>[first,
 *  last)</tt>. This version of \p reduce_into uses \p init as the initial value
 * of the reduction and \p binary_op as the binary function used for summation.
 * \p reduce_into is similar to the C++ Standard Template Library's
 * <tt>std::accumulate</tt>. The primary difference between the two functions is
 * that <tt>std::accumulate</tt> guarantees the order of summation, while \p
 * reduce_into requires associativity of
 *  \p binary_op to parallelize the reduction.
 *
 *  Note that \p reduce_into also assumes that the binary reduction operator (in
 * this case \p binary_op) is commutative. If the reduction operator is not
 * commutative then \p reduce_into should not be used. Instead, one could use \p
 * inclusive_scan (which does not require commutativity) and select the last
 * element of the output array.
 *
 ...
 *  Note that \p reduce_into also assumes that the binary reduction operator (in
 * this case \p binary_op) is commutative.  If the reduction operator is not
 * commutative then \p thrust::reduce_into should not be used.  Instead, one
 * could use
 *  \p inclusive_scan (which does not require commutativity) and select the
 *  last element of the output array.
 *
 ...
 * keys_last)</tt> that are equal, \p reduce_by_key copies the last element of
 * the group to the
 ...
 *  \return A pair of iterators at end of the ranges <tt>[keys_output,
 * keys_output_last)</tt> and <tt>[values_output, values_output_last)</tt>.
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 ...
 * href="https://en.cppreference.com/w/cpp/iterator/input_iterator">Input
 * Iterator</a>, \tparam OutputIterator1 is a model of <a
 * href="https://en.cppreference.com/w/cpp/iterator/output_iterator">Output
 * Iterator</a> and and \p InputIterator1's \c value_type is convertible to \c
 * OutputIterator1's \c value_type. \tparam OutputIterator2 is a model of <a
 * href="https://en.cppreference.com/w/cpp/iterator/output_iterator">Output
 * Iterator</a> and and \p InputIterator2's \c value_type is convertible to \c
 ...
 *  compact a sequence of key/value pairs and sum values with equal keys using
 * the \p thrust::host execution policy for parallelization:
 *
 *  \code
 *  #include <thrust/reduce.h>
 ...
 *  // The first four values in D are now {9, 21, 9, 3} and new_end.second - D
 * is 4.
 *  \endcode
 *
 *  \see reduce
 ...
 * href="https://en.cppreference.com/w/cpp/named_req/BinaryPredicate">Binary
 * Predicate</a>.
 *
 *  \pre The input ranges shall not overlap either output range.
 *
 ...
 *  such that <tt>binary_pred(*i, *(i+1))</tt> is \c true, then the
 * corresponding values are reduced to a single value with \c binary_op.
 *
 *  This version of \p reduce_by_key uses the function object \c binary_pred
 ...
 * href="https://en.cppreference.com/w/cpp/named_req/BinaryPredicate">Binary
 * Predicate</a>.
 *  \tparam BinaryFunction The function's return type must be convertible to \c
 * OutputIterator2's \c value_type.
 *
 *  \pre The input ranges shall not overlap either output range.
 *
